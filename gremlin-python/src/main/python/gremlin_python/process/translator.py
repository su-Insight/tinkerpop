...
class Translator:
    ...
    def process_predicate(self, p):
        res = ''
        p_list = []
        # connective predicates (and/or) are nested in python bytecode, e.g. or(P1, and(P2, P3))
        # so we need to unwrap them into P1.or(P2).and(P3)
        if p.operator in self.conn_p:
            res += f'{self.process_predicate(p.value)}.{p.operator}({self.process_predicate(p.other)})'
            # self.process_conn_p(p, p_list)
            # # relying on the assumption that the form of list is [P, and/or, P, and/or, P, ...][
            # for idx, it in enumerate(p_list):
            #     if idx == 0:
            #         res += f'{it}.'
            #     elif idx % 2:  # get odd indices
            #         res += f'{it}({p_list[idx+1]}).'
            #     else:
            #         pass
            # res = res[0:-1]
        else:
    ...
    # adds all the nested connective predicates in order in a list
    def process_conn_p(self, p, p_list=None):
        if p.operator in self.conn_p:
            self.process_conn_p(p.value, p_list)
            p_list.append(p.operator)
            self.process_conn_p(p.other, p_list)
        else:
            p_list.append(self.process_p_value(p))
        return p_list
