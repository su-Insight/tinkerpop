...
traversalStrategy
    : NEW? classType (LPAREN (traversalStrategyArg (COMMA traversalStrategyArg)*)? RPAREN)?
    ;

// sorta annoying that certain keywords can end up being used as configuration keys on a strategy. if they aren't
// defined here then the parser doesn't pick them up. what's the right way to fix this?!
traversalStrategyArg
    : 'keys' COLON genericLiteralArgument
    | 'edges' COLON genericLiteralArgument
